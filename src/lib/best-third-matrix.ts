// ── Official FIFA "best third-placed team" assignment matrix ───────────
//
// World Cup 2026 has 12 groups (A–L). The top 2 of each group (24 teams)
// plus the 8 best third-placed teams (out of 12 possible thirds) advance
// to the round of 32. Which 8 groups' thirds qualify is not known until
// the group stage finishes, so FIFA pre-published a fixed lookup table
// covering all C(12,8) = 495 possible combinations of qualifying groups,
// each mapping deterministically to a specific assignment of thirds to
// the 8 round-of-32 matches that face a "Best 3rd (...)" slot. This is
// Annex C of the FIFA World Cup 26 Regulations — the matrix below is NOT
// a heuristic; every entry is one specific, official, fixed assignment.
//
// Source: FIFA World Cup 26 Regulations, Annex C ("Combinations of
// matches in the round of 32"):
//   https://digitalhub.fifa.com/m/636f5c9c6f29771f/original/FWC2026_regulations_EN.pdf
// Transcribed PROGRAMMATICALLY (not by hand) from the equivalent public
// table at https://en.wikipedia.org/wiki/2026_FIFA_World_Cup_knockout_stage
// (Template:2026 FIFA World Cup third-place table) to rule out manual
// transcription error across all 495 rows. Validated automatically so
// that for every row the 8 assigned groups are exactly its 8 advancing
// groups (a true permutation, no duplicates/omissions), and every
// assigned group is one this match's placeholder actually allows.
//
// The 8 round-of-32 matches with a "Best 3rd (...)" placeholder, and the
// group whose winner they face (per the seeded fixtures in migration
// 035_knockout_fixtures.sql):
//   M74 — Winner Group E   M77 — Winner Group I   M79 — Winner Group A
//   M80 — Winner Group L   M81 — Winner Group D   M82 — Winner Group G
//   M85 — Winner Group B   M87 — Winner Group K
//
// Lookup key: the 8 advancing group letters, alphabetically sorted and
// concatenated (e.g. "EFGHIJKL"). Each value maps match_number → the
// group letter whose 3rd-placed team fills that match's Best-3rd slot.

export const _BEST_THIRD_MATRIX_ENTRIES: readonly [string, Readonly<Record<number, string>>][] = [
...
export const BEST_THIRD_MATCH_NUMBERS: readonly number[] = [74, 77, 79, 80, 81, 82, 85, 87];

export const BEST_THIRD_ASSIGNMENT_MATRIX: ReadonlyMap<string, Readonly<Record<number, string>>> =
  new Map(_BEST_THIRD_MATRIX_ENTRIES);
