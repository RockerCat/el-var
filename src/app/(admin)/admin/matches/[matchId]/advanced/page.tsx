...
import { isAdmin, getMatchForAdmin } from "@/lib/db/admin";
import AdvancedMatchEditor from "@/components/admin/AdvancedMatchEditor";
import { matchTeamCode, type Team } from "@/lib/matches";
...
export default async function AdvancedMatchEditPage({ params }: PageProps) {
  ...
  const [match, { data: teamsData }] = await Promise.all([
    getMatchForAdmin(matchId),
    supabase
      .from("teams")
